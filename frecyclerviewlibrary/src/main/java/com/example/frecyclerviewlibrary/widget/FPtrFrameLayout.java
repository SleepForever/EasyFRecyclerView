...
import com.example.frecyclerviewlibrary.ptrlib.ptr.PtrUIHandler;
import com.example.frecyclerviewlibrary.ptrlib.ptr.PtrUIHandlerHolder;
import com.example.frecyclerviewlibrary.ptrlib.ptr.PtrUIHandlerHook;
import com.example.frecyclerviewlibrary.util.AppBarStateChangeListener;
import com.example.frecyclerviewlibrary.util.LogUtil;

public class FPtrFrameLayout extends ViewGroup {
    ...
    public FPtrFrameLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray arr = context.obtainStyledAttributes(attrs, R.styleable.FPtrFrameLayout, 0, 0);
        if (arr != null) {

            mHeaderId = arr.getResourceId(R.styleable.FPtrFrameLayout_ptr_header, mHeaderId);
            mContainerId = arr.getResourceId(R.styleable.FPtrFrameLayout_ptr_content, mContainerId);

            mPtrIndicator.setResistance(
                    arr.getFloat(R.styleable.FPtrFrameLayout_ptr_resistance, mPtrIndicator.getResistance()));

            mDurationToClose = arr.getInt(R.styleable.FPtrFrameLayout_ptr_duration_to_close, mDurationToClose);
            mDurationToCloseHeader = arr.getInt(R.styleable.FPtrFrameLayout_ptr_duration_to_close_header, mDurationToCloseHeader);

            float ratio = mPtrIndicator.getRatioOfHeaderToHeightRefresh();
            ratio = arr.getFloat(R.styleable.FPtrFrameLayout_ptr_ratio_of_header_height_to_refresh, ratio);
            mPtrIndicator.setRatioOfHeaderHeightToRefresh(ratio);

            mKeepHeaderWhenRefresh = arr.getBoolean(R.styleable.FPtrFrameLayout_ptr_keep_header_when_refresh, mKeepHeaderWhenRefresh);

            mPullToRefresh = arr.getBoolean(R.styleable.FPtrFrameLayout_ptr_pull_to_fresh, mPullToRefresh);
            arr.recycle();
        }
