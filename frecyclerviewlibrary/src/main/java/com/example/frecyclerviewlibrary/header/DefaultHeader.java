package com.example.frecyclerviewlibrary.header;
...
public class DefaultHeader extends FrameLayout implements PtrUIHandler {
    ...
    public void onUIPositionChange(FPtrFrameLayout frame, boolean isUnderTouch, byte status, PtrIndicator ptrIndicator) {
        ...
        if (currentPos < mOffsetToRefresh && lastPos >= mOffsetToRefresh) {
            if (isUnderTouch && status == FPtrFrameLayout.PTR_STATUS_PREPARE) {
    ...
    private void crossRotateLineFromTopUnderTouch(FPtrFrameLayout frame) {
        if (!frame.isPullToRefresh()) {
            mTitleTextView.setVisibility(VISIBLE);
    ...
    private void crossRotateLineFromBottomUnderTouch(FPtrFrameLayout frame) {
        mTitleTextView.setVisibility(VISIBLE);
        if (frame.isPullToRefresh()) {
            mTitleTextView.setText(getResources().getString(R.string.cube_ptr_pull_down_to_refresh));
        } else {
